...
'''Transform yolo format to Pascal VOC Format'''
''' WARNING: cam obviously not be used for resized images, since coords of BBox will be changed'''


folder_holding_yolo_files = "D:\\Master_Daten\\sundatabase_yolo_split\\labels\\val" # input("Enter the path to the yolo files: ").replace("'", "").strip()
...
for each_yolo_file in os.listdir(folder_holding_yolo_files):
    if each_yolo_file.endswith("txt"):
        ...
        if os.path.exists(os.path.join(folder_holding_yolo_files.replace("labels", "images"), each_yolo_file.replace('txt', 'jpeg'))):
            image_name = each_yolo_file.replace('txt', 'jpeg')
        if os.path.exists(os.path.join(folder_holding_yolo_files.replace("labels", "images"), each_yolo_file.replace('txt', 'jpg'))):
            image_name = each_yolo_file.replace('txt', 'jpg')
        if os.path.exists(os.path.join(folder_holding_yolo_files.replace("labels", "images"), each_yolo_file.replace('txt', 'png'))):
        ...
        if not image_name == each_yolo_file:
            ...
            orig_img = Image.open(os.path.join(folder_holding_yolo_files.replace("labels", "images"), image_name)) # open the image
            image_width = orig_img.width
            image_height = orig_img.height

            # Start the XML file
            with open('XML' + os.sep + each_yolo_file.replace('txt', 'xml'), 'w') as f:
                #print("write: ", 'XML' + os.sep + each_yolo_file.replace('txt', 'xml'))
                f.write('<annotation>\n')
